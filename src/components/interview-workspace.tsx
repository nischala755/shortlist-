...
type FeedbackSummary = { provider: string; model: string; disclaimer: string; summary: { summary: string; strengths: string[]; concerns: string[]; followUpQuestions: string[] } };
...
export function InterviewWorkspace({ organizationId, organizationName, canSchedule, canScore, applications, members }: { organizationId: string; organizationName: string; canSchedule: boolean; canScore: boolean; applications: Application[]; members: Member[] }) {
  ...
  async function summarizeFeedback(applicationId: string, interviewId: string) {
    const path = `/api/organizations/${organizationId}/applications/${applicationId}/interviews/${interviewId}/scorecard/summary`;
    setBusy(path); setError("");
    try {
      const response = await fetch(path, { method: "POST" });
      if (!response.ok) { setError(await responseError(response)); return; }
      const result = (await response.json()) as FeedbackSummary;
      setFeedbackSummaries((current) => ({ ...current, [interviewId]: result }));
    } catch { setError("The service is unavailable. Try again shortly."); }
    finally { setBusy(""); }
  }
  ...
  return <main className="workspace-main interview-main"><div className="workspace-title"><div><p className="eyebrow">{organizationName}</p><h1>Interviews</h1><p>Schedule structured conversations, capture consistent feedback, and keep the decision with the hiring team.</p></div>{canSchedule && selected && <button className="button primary" type="button" onClick={() => setEditor({ application: selected })}>Schedule interview</button>}</div>
    ...
    <div className="interview-layout"><aside><label>Application<select value={selected?.id ?? ""} onChange={(event) => setApplicationId(event.target.value)}><option value="" disabled>Select application</option>{applications.map((application) => <option key={application.id} value={application.id}>{application.candidate.name} / {application.job.title}</option>)}</select></label><h2>Upcoming schedule</h2>{upcoming.length ? <ul>{upcoming.map((item) => <li key={item.id}><strong>{new Intl.DateTimeFormat("en", { day: "numeric", month: "short", hour: "numeric", minute: "2-digit" }).format(new Date(item.scheduledStart))}</strong><span>{item.application.candidate.name}</span><small>{item.interviewer.email}</small></li>)}</ul> : <p>No upcoming interviews.</p>}</aside><section className="interview-detail">{selected ? <><header><div><span>{selected.currentStage.toLowerCase()}</span><h2>{selected.candidate.name}</h2><p>{selected.job.title} / {selected.candidate.email}</p></div></header>{selected.interviews.length ? <div className="interview-list">{selected.interviews.map((interview) => { const feedbackSummary = feedbackSummaries[interview.id]; return <article key={interview.id}><header><div><span className={`interview-status ${interview.status.toLowerCase()}`}>{interview.status.toLowerCase()}</span><h3>{new Intl.DateTimeFormat("en", { dateStyle: "medium", timeStyle: "short" }).format(new Date(interview.scheduledStart))}</h3><p>{interview.interviewer.email}</p></div><strong>{Math.round((new Date(interview.scheduledEnd).getTime() - new Date(interview.scheduledStart).getTime()) / 60000)} min</strong></header><div className="interview-access">{interview.meetingUrl ? <a href={interview.meetingUrl} target="_blank" rel="noreferrer">Open meeting link</a> : <span>{interview.location}</span>}</div>{interview.scorecard ? <div className="scorecard-review"><div><span>Overall rating</span><strong>{interview.scorecard.overallRating}/5</strong></div><p><b>Strengths</b>{interview.scorecard.strengths || "Not recorded"}</p><p><b>Concerns</b>{interview.scorecard.concerns || "Not recorded"}</p><small>Submitted by {interview.scorecard.submittedBy.email}</small></div> : <p className="scorecard-empty">No structured scorecard submitted.</p>}{feedbackSummary && <section className="feedback-summary"><div><p className="eyebrow">AI-assisted summary</p><h4>Structured feedback overview</h4></div><p>{feedbackSummary.summary.summary}</p>{feedbackSummary.summary.strengths.length > 0 && <><strong>Recorded strengths</strong><ul>{feedbackSummary.summary.strengths.map((item) => <li key={item}>{item}</li>)}</ul></>}{feedbackSummary.summary.concerns.length > 0 && <><strong>Recorded concerns</strong><ul>{feedbackSummary.summary.concerns.map((item) => <li key={item}>{item}</li>)}</ul></>}{feedbackSummary.summary.followUpQuestions.length > 0 && <><strong>Follow-up questions</strong><ul>{feedbackSummary.summary.followUpQuestions.map((item) => <li key={item}>{item}</li>)}</ul></>}<small>{feedbackSummary.disclaimer} {feedbackSummary.provider} / {feedbackSummary.model}</small></section>}<footer>{canSchedule && interview.status !== "CANCELLED" && <button type="button" onClick={() => setEditor({ application: selected, interview })}>{interview.status === "SCHEDULED" ? "Reschedule" : "Edit details"}</button>}{canSchedule && interview.status === "SCHEDULED" && <button className="danger" type="button" disabled={Boolean(busy)} onClick={() => request(`/api/organizations/${organizationId}/applications/${selected.id}/interviews/${interview.id}`, { method: "DELETE" })}>Cancel interview</button>}{canScore && interview.status === "COMPLETED" && <button className="primary-action" type="button" onClick={() => setScoreEditor({ application: selected, interview })}>{interview.scorecard ? "Edit scorecard" : "Add scorecard"}</button>}{interview.scorecard && <button type="button" disabled={Boolean(busy)} onClick={() => summarizeFeedback(selected.id, interview.id)}>{feedbackSummary ? "Summarize again" : "Summarize feedback"}</button>}</footer></article>; })}</div> : <div className="panel-empty">No interviews scheduled for this application.</div>}</> : <div className="panel-empty">No applications are available.</div>}</section></div>
