...
Browser routes include the product landing page, registration, email verification, login, password recovery, and protected workspaces under `/dashboard`. The organization workspace supports the complete job lifecycle: draft creation and editing, evidence requirements, publishing, filtering, and closing. Its candidate workspace supports organization-scoped search, profile creation and editing, PDF or DOCX resume upload and parsing, and application creation for published jobs. The pipeline board remains a separate vertical slice over the existing APIs.
